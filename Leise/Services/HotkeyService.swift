...
final class HotkeyService: ObservableObject {
    ...
    var onDictationStart: ((UInt64) -> Void)? { didSet { flushPendingDictationActionsIfReady() } }
    var onDictationStop: (() -> Void)? { didSet { flushPendingDictationActionsIfReady() } }
    var onRecentTranscriptionsToggle: (() -> Void)?
    var onCopyLastTranscription: (() -> Void)?
    var onRecorderToggle: (() -> Void)?
    var onProfileDictationStart: ((UUID, UInt64) -> Void)? { didSet { flushPendingDictationActionsIfReady() } }
    ...
    private static let carbonHotkeySignature: OSType = "leis".utf16.reduce(0) { ($0 << 8) + OSType($1) }
    ...
    private enum PendingDictationAction {
        case start(UInt64)
        case stop
        case profile(UUID, UInt64)
    }
    ...
    private func dispatchDictationStart(_ timestamp: UInt64) {
        guard let onDictationStart else {
            pendingDictationActions.append(.start(timestamp))
            return
        }
        onDictationStart(timestamp)
    }

    private func dispatchDictationStop() {
        guard let onDictationStop else {
            pendingDictationActions.append(.stop)
            return
        }
        onDictationStop()
    }

    private func dispatchProfileDictationStart(_ profileID: UUID, timestamp: UInt64) {
        guard let onProfileDictationStart else {
            pendingDictationActions.append(.profile(profileID, timestamp))
            return
        }
        onProfileDictationStart(profileID, timestamp)
    }

    private func flushPendingDictationActionsIfReady() {
        guard onDictationStart != nil,
              onDictationStop != nil,
              onProfileDictationStart != nil,
              !pendingDictationActions.isEmpty else {
            return
        }

        let actions = pendingDictationActions
        pendingDictationActions.removeAll(keepingCapacity: true)
        for action in actions {
            ...
            case .stop:
                onDictationStop?()
            case .profile(let profileID, let timestamp):
                onProfileDictationStart?(profileID, timestamp)
            }
        }
    }
    ...
    #if DEBUG
    func dispatchDictationStartForTesting(_ timestamp: UInt64) {
        dispatchDictationStart(timestamp)
    }

    func dispatchDictationStopForTesting() {
        dispatchDictationStop()
    }
    #endif
    ...
    /// Returns true if the event should be suppressed (consumed by Leise).
    ...
    private func activatePendingHybridModifierHold(generation: UInt64) {
        ...
        dispatchDictationStart(Self.requestTimestamp())
    }
    ...
    private func handleKeyDown(slotType: HotkeySlotType, hotkey: UnifiedHotkey) {
        ...
        if isActive {
            ...
            dispatchDictationStart(requestTimestamp)
        }
    }
    ...
    private func handleProfileKeyDown(profileId: UUID) {
        if isActive {
            ...
            dispatchProfileDictationStart(profileId, timestamp: requestTimestamp)
        }
    }
