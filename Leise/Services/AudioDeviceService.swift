...
private let logger = Logger(subsystem: Bundle.main.bundleIdentifier ?? "leise-mac", category: "AudioDeviceService")
...
final class AudioDeviceService: ObservableObject, @unchecked Sendable {
    ...
    private let previewNotificationQueue: OperationQueue = {
        ...
        queue.name = "com.leise.preview-recovery.notifications"
    ...
    private let previewRecoveryQueue = DispatchQueue(label: "com.leise.preview-recovery", qos: .userInitiated)
    ...
    /// See issue #332.
    private let previewEngineTeardownRetainer = DelayedReleaseRetainer<AVAudioEngine>(label: "com.leise.preview-engine-teardown")
    ...
    private static func inputDeviceDiagnostics(
        ...
        return AudioInputDiagnosticsReport.Device(
            ...
            listedByLeise: listedDevice != nil,
...
private let deviceHelperLogger = Logger(subsystem: Bundle.main.bundleIdentifier ?? "leise-mac", category: "AudioDeviceHelper")
...
final class CoreAudioHALInputCaptureSession: AudioInputCaptureSession, @unchecked Sendable {
    ...
    private final class CallbackLifetime: @unchecked Sendable {
        private static let teardownQueue = DispatchQueue(
            label: "com.leise.coreaudio-hal-teardown",
            qos: .utility
        )
        let audioUnit: AudioUnit
        let operations: CoreAudioHALInputOperating
