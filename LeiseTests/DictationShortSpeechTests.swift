...
final class DictationShortSpeechTests: XCTestCase {
    ...
    func testDelayedReleaseRetainer_keepsObjectAliveUntilDelayExpires() throws {
        let retainer = DelayedReleaseRetainer<ReleaseProbe>(label: "com.leise.tests.delayed-release")
...
final class DictationInsertionTextFormatterTests: XCTestCase {
    ...
    func testSmartInsertionLowercasesTitleCaseFirstWord() {
        let context = TextInsertionService.InsertionContext(
            value: "about",
            selectedRange: NSRange(location: 5, length: 0),
            selectedText: nil,
        ...
        XCTAssertEqual(
            DictationInsertionTextFormatter.textForInsertion("Leise", insertionContext: context),
            " leise"
        )
    }
